...
- [ ] **Regulations / ComplianceSystem is entirely dead.** `consequences/regulations.rs` defines 8 `RegulationType`s with $1,000–$5,000 fines and inspection schedules; `init`/`tick` are called — but `add_violation` is `#[cfg(test)]`-only, no code path ever schedules an inspection or creates a violation, and `unpaid_fines`/`compliance_reputation` are never read. Wiring this in gives the economy a real antagonist (surprise inspections, escalating fines for neglect) and directly attacks "too easy to make money."
- [ ] **Half the narrative effect types are silently dropped.** `state/gameplay_effects.rs:63` has a `_ => {}` catch-all that discards 6 of 12 `NarrativeEffect` variants: `NeighborhoodReputation`, `BuildingHappiness`, `EconomyChange`, `RentDemand`, `TriggerInspection`, `PropertyValue`. Every neighborhood/city news event (New Business, Crime Report, Housing Market…) is pure flavor. Implement the six handlers.
- [ ] **Reputation is displayed and scored but cannot be moved by the player.** `choice.reputation_change` is rendered on event buttons ("Rep +5", `ui/event_modal.rs:61`) but never applied (`gameplay_actions.rs:511` applies only `effect`); `MissionReward::Reputation` shows floating text without changing anything (`mission_system.rs:118-126`); `NeighborhoodReputation` effects are dropped (above). Yet reputation feeds career score and mission rewards. Make reputation a real currency: sources (event choices, mission rewards, compliance), sinks, and consequences (applicant quality/volume, rent ceiling tolerance, event frequency).
- [ ] **Tenant life events (the emergent-story engine) have no generator.** `narrative/stories.rs` models 10 rich `LifeChangeType`s (NewJob, JobLoss, NewBaby, Partnered, Retired, Graduated…) but nothing generates them and `StoryImpact::LifeChange` is an explicit no-op (`gameplay_actions.rs:525`). This is the single best content-per-effort investment: life events that change income/noise/space needs create exactly the emergent dilemmas the playtest praised ("Sarah pays a lot but makes others unhappy").
