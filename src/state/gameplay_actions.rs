     ...
     pub(super) fn process_action(&mut self, action: UiAction) {
         match action {
             ...
             UiAction::UpgradeAction(upgrade) => {
                 let description = upgrade.label(&self.building, &self.config.ui, &self.config.upgrades);
                 if let Ok(cost) = process_upgrade(&upgrade, &mut self.building, &mut self.funds, &self.config, self.current_tick) {
